...
namespace StiLib.Vision
{
    ...
    public struct PrimitivePara
    {
        ...
        public static PrimitivePara Default
        {
            get
            {
                return Point(Vector3.Zero, Color.White);
            }
        }
        ...
        /// <summary>
        /// Gets a Point Primitive Parameters
        /// </summary>
        /// <param name="center"></param>
        /// <param name="color"></param>
        /// <returns></returns>
        public static PrimitivePara Point(Vector3 center, Color color)
        {
            return new PrimitivePara(new VertexPositionColor[] { new VertexPositionColor(center, color) }, new int[] { 0 }, vsBasePara.VSTypeDefault(VSType.Primitive));
        }
        ...
        /// <param name="color"></param>
        /// <returns></returns>
        public static PrimitivePara Line(Vector3 center, Vector3 startpoint, Vector3 endpoint, Color color)
        {
            return Line(center, startpoint, color, endpoint, color);
        }
        ...
        /// <summary>
        /// Gets a Triangle Primitive Parameters
        /// </summary>
        /// <param name="center"></param>
        /// <param name="color"></param>
        ...
        /// <returns></returns>
        public static PrimitivePara Triangle(Vector3 center, Color color, Vector3 point0,Vector3 point1,Vector3 point2, bool isfilled)
        {
            if(isfilled)
            {
                var temp = LineStrip(false, center, color, point0, point1, point2);
                temp.BasePara.primitivetype = PrimitiveType.TriangleList;
                return temp;
            }
            else
            {
                return LineStrip(true,center, color, point0, point1, point2);
            }
        }

        /// <summary>
        /// Gets a Rectangle Primitive Parameters
        /// </summary>
        /// <param name="center"></param>
        /// <param name="color"></param>
        /// <param name="width"></param>
        ...
        /// <param name="isfilled"></param>
        /// <returns></returns>
        public static PrimitivePara Rectangle(Vector3 center,Color color,float width,float height,bool isfilled)
        {
            if(isfilled)
            {
                var temp = LineStrip(false, center, color, new Vector3(-width / 2f, -height / 2f, 0),
                                 new Vector3(-width / 2f, height / 2f, 0), new Vector3(width / 2f, height / 2f, 0),
                                 new Vector3(width / 2f, -height / 2f, 0));
                temp.BasePara.primitivetype = PrimitiveType.TriangleFan;
                return temp;
            }
            else
            {
                return LineStrip(true, center, color, new Vector3(-width/2f, -height/2f, 0),
                                 new Vector3(-width/2f, height/2f, 0), new Vector3(width/2f, height/2f, 0),
                                 new Vector3(width/2f, -height/2f, 0));
            }
        }
        ...
        /// <summary>
        /// Gets a Quadrate Primitive Parameters
        /// </summary>
        /// <param name="center"></param>
        /// <param name="color"></param>
        ...
        /// <param name="isfilled"></param>
        /// <returns></returns>
        public static PrimitivePara Quadrate(Vector3 center,Color color,float size,bool isfilled)
        {
            return Rectangle(center, color, size, size, isfilled);
        }

        /// <summary>
        /// Gets a Ellipse Primitive Parameters
        /// </summary>
        /// <param name="longdiameter"></param>
        /// <param name="shortdiameter"></param>
        /// <param name="color"></param>
        /// <param name="center"></param>
        ...
        /// <param name="isfilled"></param>
        /// <returns></returns>
        public static PrimitivePara Ellipse(float longdiameter, float shortdiameter,Color color, Vector3 center,Color centercolor, int resolution, bool isfilled)
        {
            VertexPositionColor[] v;
            PrimitiveType type;
            int[] index;
            ...
            if (isfilled)
            {
                ...
                {
                    v[i].Position.X = (float)Math.Sin(res_rad * (i - 1)) * longdiameter / 2;
                    v[i].Position.Y = (float)Math.Cos(res_rad * (i - 1)) * shortdiameter / 2;
                    v[i].Position.Z = 0.0f;
                    v[i].Color = color;
                    index[i] = i;
                }
            ...
            else
            {
                ...
                {
                    v[i].Position.X = (float)Math.Sin(res_rad * i) * longdiameter / 2;
                    v[i].Position.Y = (float)Math.Cos(res_rad * i) * shortdiameter / 2;
                    v[i].Position.Z = 0.0f;
                    v[i].Color = color;
                    index[i] = i;
                }
            ...
            return new PrimitivePara(v, index, center, 0.0f, 0.0f, Vector3.Zero, 10.0f, Vector3.Zero, 0.0f, 10.0f, Vector3.Zero, 10.0f, color, true, "BasicEffect", 1.0, type);
        }

        /// <summary>
        /// Gets a Ellipse Primitive Parameters with uniform color and resolution = 100 points in 2*PI
        /// </summary>
        /// <param name="longdiameter"></param>
        /// <param name="shortdiameter"></param>
        /// <param name="color"></param>
        ...
        /// <param name="isfilled"></param>
        /// <returns></returns>
        public static PrimitivePara Ellipse(float longdiameter, float shortdiameter, Color color, Vector3 center, bool isfilled)
        {
            return Ellipse(longdiameter, shortdiameter, color, center, color, 100, isfilled);
        }

        /// <summary>
        /// Gets a Circle Primitive Parameters with uniform color and center = (0, 0, 0), resolution = 100 points in a circle
        /// </summary>
        /// <param name="diameter"></param>
        /// <param name="color"></param>
        /// <param name="isfilled"></param>
        ...
        {
            return Circle(diameter, color, Vector3.Zero,color, 100, isfilled);
        }
        ...
        /// <param name="isfilled"></param>
        /// <returns></returns>
        public static PrimitivePara Circle(float diameter, Color color, Vector3 center, Color centercolor, int resolution, bool isfilled)
        {
            var temp = Ellipse(diameter, diameter, color, center, centercolor, resolution, isfilled);
            temp.BasePara.diameter = diameter;
            return temp;
        }

        /// <summary>
        /// Gets a Quadrate Grid Primitive Parameters
        /// </summary>
        ...
        /// <param name="color"></param>
        /// <returns></returns>
        public static PrimitivePara Grid(float interval,int resolution,Vector3 center, Color color)
        {
            return Grid(interval, interval, resolution, resolution, center, color, color);
        }

        /// <summary>
        /// Gets a Grid Primitive Parameters
        /// </summary>
        /// <param name="rowinterval"></param>
        /// <param name="columninterval"></param>
        /// <param name="rows"></param>
        /// <param name="columns"></param>
        /// <param name="center"></param>
        /// <param name="rowcolor"></param>
        /// <param name="columncolor"></param>
        /// <returns></returns>
        public static PrimitivePara Grid(float rowinterval,float columninterval,int rows,int columns,Vector3 center,Color rowcolor,Color columncolor)
        {
            if (rows % 2 == 0)
            {
                rows += 1;
            ...
            float upperleft_x = -(columns - 1)*columninterval/2;
            float upperleft_y = (rows - 1)*rowinterval/2;
            float width = (columns - 1)*columninterval;
            float height = (rows - 1)*rowinterval;

            VertexPositionColor[] v = new VertexPositionColor[rows*2+columns*2];
            int[] i = new int[rows*2+columns*2];

            int start_end, step;
            for (int j = 0; j < rows * 2; j++)
            {
                start_end = j%2;
                step = (int)Math.Floor(j/2.0);
                v[j].Position = new Vector3(upperleft_x+start_end*width, upperleft_y-step*rowinterval, 0.0f);
                v[j].Color =rowcolor;
                i[j] = j;
            }
            for (int j = rows * 2; j < v.Length; j++)
            {
                start_end = j % 2;
                step = (int)Math.Floor(j / 2.0)-rows;
                v[j].Position = new Vector3(upperleft_x + step * columninterval, upperleft_y - start_end * height, 0.0f);
                v[j].Color = columncolor;
                i[j] = j;
            }

            return new PrimitivePara(v, i, center, 0.0f, 0.0f, Vector3.Zero, 10.0f, Vector3.Zero, 0.0f, 10.0f, Vector3.Zero, 10.0f, Color.White, true, "BasicEffect", 1.0, PrimitiveType.LineList);
        }
