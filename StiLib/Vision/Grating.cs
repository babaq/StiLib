...
namespace StiLib.Vision
{
    ...
    public class Grating : VisionStimulus
    {
        ...
        public void FillGrating(bool ischeckresolution)
        {
            ...
            int interval_n = 2*(int) Math.Round(radius/interval);
            vertexN = 2 * (interval_n + 1);
            radius = interval_n*interval/2.0;
            ...
            for (int i = 0; i < vertexN; i++)
            {
                ...
                if (Xstep - radius >= radius) Xstep = radius*2.0-0.000001;
                ...
                if (updown == 0) // Up
                {
                    ...
                    {
                        case Shape.Circle:
                            Ycoor = Math.Sqrt(Math.Pow(radius, 2.0) - Math.Pow((radius - Xstep), 2.0)); break;
                        default: // Quadrate
                            Ycoor = radius; break;
                    }
                }
                ...
                vertexArray[i].Position = new Vector3((float)(-radius + Xstep), (float)Ycoor, 0.0f);
                indexArray[i] = i;
            }
