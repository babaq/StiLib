// F# Script File : Dir_Topology_IOI.fsx
//
// Mapping Topology Relationship between Visual Field and Direction/Orientation Map in intransic optic imaging
//
// Copyright (c) 2010-01-26 Zhang Li
    ...
    member this.Update_Grating() = 
        ...
        if this.ex.PPort.Timer.ElapsedSeconds < float this.ex.Exdesign.durT then
            if this.ex.Flow.IsPred = false then
                ...
                let rad = Math.Floor(float (this.ex.Flow.StiCount / (this.ex.Cond.[1].VALUE.ValueN * this.ex.Cond.[1].VALUE.ValueN))) * (Math.PI / float this.ex.Cond.[0].VALUE.ValueN)
                let loci = this.ex.Flow.StiCount % (this.ex.Cond.[1].VALUE.ValueN * this.ex.Cond.[1].VALUE.ValueN)
                
                let RCount = Math.Floor(float (loci / this.ex.Cond.[1].VALUE.ValueN))
                let CCount = loci % this.ex.Cond.[1].VALUE.ValueN
                
                let Xgrid = -float32(this.ex.Cond.[1].VALUE.ValueN - 1) * this.step / 2.0f + this.step * float32(CCount)
                let Ygrid = float32(this.ex.Cond.[1].VALUE.ValueN - 1) * this.step / 2.0f - this.step * float32(RCount)
                this.grating.Ori3DMatrix <- Matrix.CreateRotationZ(float32 rad)
...
let MyExperiment = new MyEx(Text = "F# Scripting Dir_Topology_IOI")
